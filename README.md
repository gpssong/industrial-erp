...
**新功能**: 销售出库新增/编辑弹窗, 选定客户后底部增加 "历史销售" 列表
- 列出该客户最近 50 条销售出库明细 (按出库日期 DESC), 含主表 billNo/billDate 冗余
- 列: 产品编码 / 名称 / 规格 / 单位 / 数量 / 单价(含税) / 备注
- 单击行 (或点击 "加入明细" 按钮) → 把该明细加到上方明细行 (同 productId+batchNo 自动合并数量)
...
**用户报 2**: `home.93gushi.com:8088` 报 502 Bad Gateway

**根因**: DSM 自动生成的 `/etc/nginx/conf.d/http.erp8088.conf` 把 `8088` 的 `location /` 反代到 `http://127.0.0.1:18080` (在构建初期 pc-web 容器就绑的端口)。早先重启时我误把容器端口改成 `8189`,导致 8088 反代上游没了 → 502。`8189` 是空端口(公网不通),用户访问 8088 自然 502。

**修复**: 把 `erp-pc-web` 容器端口改回 `18080:80`,反代链路恢复

```bash
docker rm -f erp-pc-web && \
docker run -d --name erp-pc-web --restart=always \
  --network erp-system_erp-net -p 18080:80 erp-system-pc-web:1.1.7
```

**验证**: `curl -o /dev/null -w "%{http_code}\n" http://home.93gushi.com:8088/` → `200`
