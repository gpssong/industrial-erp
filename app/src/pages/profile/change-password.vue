...
// 不 import api, 改用 uni.request 直接调用, 避免 vite tree-shaking 删掉方法
...
async function onSubmit() {
  ...
  try {
    const res = await uni.request({ url: 'http://home.93gushi.com:8088/api/system/user/me/password', method: 'PUT', header: { 'Authorization': uni.getStorageSync('erp_token') || '', 'Content-Type': 'application/json' }, data: { oldPassword: form.oldPassword, newPassword: form.newPassword } })
    if (res.statusCode === 200) {
      toast('修改成功, 请重新登录')
    } else {
      toast('修改失败: ' + (res.data && res.data.msg || res.statusCode))
    }
    // 清除 token 跳转登录
