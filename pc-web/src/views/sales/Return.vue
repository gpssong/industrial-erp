      ...
      <el-table :data="data.records" border stripe v-loading="loading">
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.billStatus==='DRAFT' && userStore.hasPerm('sales:return:check')" link type="primary" @click="onCheck(row)">审核</el-button>
            <el-button v-if="row.billStatus==='CHECKED' && userStore.hasPerm('sales:return:uncheck')" link type="warning" @click="onUncheck(row)">反审核</el-button>
...
async function onUncheck(row) {
  try {
    await ElMessageBox.confirm(
      `确认反审核销售退货单 ${row.billNo}?\n\n此操作仅回退单据状态至「草稿」, 不会自动回退已入库的库存或冲减的应收.\n`,
      '反审核确认', { type: 'warning', confirmButtonText: '确认反审核', cancelButtonText: '取消' }
    )
  } catch { return }
  try {
    await salReturnApi.uncheck(row.id); ElMessage.success('反审核成功'); loadData()
  } catch (e) {
