...
<#if (bill).address?? && (bill).address?has_content><div class="row"><span>地址: ${(bill).address}</span></div></#if>
<#if (bill).phone?? && (bill).phone?has_content><div class="row"><span>电话: ${(bill).phone}</span></div></#if>
<table>
  <thead>
    <tr>
